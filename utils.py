...
def calculate_stats(trend_data):
    """Calcula las estadísticas de los datos de tendencia, incluyendo el percentil 95"""
    if not trend_data:
    ...
    percentile_95 = np.percentile(avg_values, 95)
    
    return mean(avg_values), min(min_values), max(max_values), percentile_95
