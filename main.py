...
def process_items(zabbix, host_id, item_type, time_from, time_till, item_keys):
    ...
    for percent_item in percent_items:
        ...
        avg, min_val, max_val, percentile_95 = calculate_stats(trend_data)
        result.update({
            ...
            'Percentil 95 Porcentaje': f"{percentile_95:.2f}%" if percentile_95 is not None else "N/A"
        })
        ...
        if used_items:
            ...
            if used_item:
                ...
                result.update({
                    ...
                    'Percentil 95 Usado': bytes_to_human_readable(percentile_95) if percentile_95 is not None else "N/A"
                })
            else:
                result.update({
                    'Espacio Usado': 'N/A',
...
def generate_report(config_file, start_date, end_date, report_name=None):
    ...
    send_email(config, output_file, start_date, end_date)
